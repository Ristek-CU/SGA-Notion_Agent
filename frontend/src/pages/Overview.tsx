    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4 sm:gap-6">
        <div className="bg-white p-4 sm:p-6 rounded-xl border border-slate-200 shadow-xs">
          <h2 className="text-base sm:text-lg font-semibold text-slate-900 mb-4">Tickets by Division</h2>
            ...
            <div className="space-y-3">
              {Object.entries(overview?.by_division || {}).map(([div, count]) => (
                <div key={div} className="flex justify-between items-center pb-2 border-b border-slate-100">
                  <span className="text-xs sm:text-sm font-medium text-slate-700">{div}</span>
                  <span className="text-xs sm:text-sm font-bold text-slate-900">{count as number}</span>
                </div>
              ))}
            </div>
          )}
        ...
        <div className="bg-white p-4 sm:p-6 rounded-xl border border-slate-200 shadow-xs">
          <h2 className="text-base sm:text-lg font-semibold text-slate-900 mb-4">Tickets by Status</h2>
