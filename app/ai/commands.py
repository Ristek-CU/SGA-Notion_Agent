...
COMMAND_PATTERNS = [
    ...
    ("my_profile", r"^(?:tolong\s+|bisa\s+)?(?:cek\s+|lihat\s+)?(?:profil|profile|data\s+diri)(?:\s+saya|\s+ku)?$"),
    ("edit_profile_name", r"^(?:tolong\s+|bisa\s+)?(?:ganti|ubah|update|edit)\s+(?:nama\s+lengkap|nama)(?:\s+saya|\s+ku)?(?:\s+(?:menjadi|jadi|ke|to|=))?\s+(.+)$"),
